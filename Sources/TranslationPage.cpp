...
void TranslationPage::setupUI()
{
    ...
    m_sourceLangCombo->setStyleSheet(
        "QComboBox {"
        "  background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 rgba(20, 30, 60, 220), stop:1 rgba(12, 20, 45, 220));"
        "  color: #eef4ff;"
        "  border: 1px solid rgba(101, 126, 255, 40);"
        "  border-radius: 10px;"
        "  font-size: 13px;"
        "  padding: 9px 14px;"
        "}"
        "QComboBox:hover {"
        "  border-color: rgba(101, 126, 255, 100);"
        "  background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 rgba(25, 38, 75, 230), stop:1 rgba(15, 25, 55, 230));"
        "}"
        "QComboBox::drop-down {"
        "  border: none;"
        "  width: 36px;"
        ...
        "  image: url(:/icons/arrow_down_blue.svg);"
        "  width: 12px; height: 12px;"
        "}"
        "QComboBox::down-arrow:on {"
        "  image: url(:/icons/arrow_up_blue.svg);"
        "}"
        ...
        "  background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 rgba(101, 126, 255, 0.2), stop:1 rgba(101, 126, 255, 0.05));"
        "  color: #8b9cf7;"
        "}"
        "QComboBox QAbstractItemView::item:hover {"
        "  background: rgba(101, 126, 255, 0.1);"
        "  color: #ffffff;"
        "}");
...
static QWidget *createCard(QWidget *parent, DashboardPage *db) {
    auto u = createUnifiedCard(parent, QString::fromUtf8("\xe7\xbf\xbb\xe8\xaf\x91"), db, "translate", false);
    ...
    auto *translateBtn = new QPushButton(QString::fromUtf8("\xe7\xbf\xbb\xe8\xaf\x91"));
    translateBtn->setCursor(Qt::PointingHandCursor);
    ...
    u.bottomRow->addStretch(1);
    u.bottomRow->addWidget(translateBtn);
